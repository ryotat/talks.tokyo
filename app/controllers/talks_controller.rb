...
class TalksController < ApplicationController
  ...
  def set_cal_path
    if ['new','edit'].include? params[:action]
      @cal_path = list_path(@talk.series, :period => 'upcoming', :date => @talk.start_time.strftime('%Y/%m/%d'), :target => 'input#talk_date_string', :format => 'calendar_with_talks', :trigger =>'hover')
    else
      @cal_path = list_path(@talk.series, :date => @talk.start_time.strftime('%Y/%m/%d'), :target => '#talks-calendar', :format => 'calendar_with_talks', :trigger =>'click')
    end
  end
